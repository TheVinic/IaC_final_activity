resource "aws_db_subnet_group" "main" {
  name       = "dbsubnetgroup-mysql-${lower(terraform.workspace)}"
  subnet_ids = [for k, v in aws_subnet.private : aws_subnet.private[k].id]
  tags = {
    Name = "${var.project}-dbsubnetgroup-mysql-${lower(terraform.workspace)}"
  }
}
...
resource "aws_db_instance" "main" {
  ...
  tags = {
    Name = "${var.project}-rds-mysql-${lower(terraform.workspace)}"
  }
}
...
resource "aws_db_instance" "replica" {
  ...
  backup_retention_period  = terraform.workspace == "Prd" ? 30 : 7
  identifier               = "rds-mysqlreplica-${lower(terraform.workspace)}"
  instance_class           = "db.t3.micro"
  publicly_accessible      = false
  storage_encrypted        = true
  storage_type             = "gp3"
  ...
  tags = {
    Name = "${var.project}-rds-mysqlreplica-${lower(terraform.workspace)}"
  }
}